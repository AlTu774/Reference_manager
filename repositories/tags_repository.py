...
def insert_tag(user_id, tag_name):
    ...
    db.session.execute(text(sql), {"user_id":user_id, "tag_name":tag_name})
    db.session.commit()
    return True
...
def tag_book(tag_id, book_id):
    ...
    db.session.execute(text(sql), {"tag_id":tag_id, "book_id":book_id})
    db.session.commit()
    return True

def get_tags_by_user_id(user_id):
    sql = """SELECT id, tag_name FROM tags WHERE creator_id=:user_id"""
    taglist = db.session.execute(text(sql), {"user_id":user_id}).fetchall()
...
def get_tags_by_book_id(book_id):
    ...
    tags_of_book = db.session.execute(text(sql), {"book_id":book_id}).fetchall()
